...
class HelperFractionTest extends TestCase
{
    /**
    ...
    public function testGet($departement)
    {
        foreach (Entries::CATEGORIES_CEE as $key => $value) {
            $this->assertEquals(
                HelperFractionClassique::get($value, $departement) 
                + HelperFractionPrecarite::get($value, $departement) 
                + HelperFractionGrandePrecarite::get($value, $departement),
                (float) 1
            );
        }
    }
